...
Protocolo ASCII con delimitadores <...> (según firmware real del myRIO).

Status (myRIO -> RPi):
  <A<int>B<int>C<int>D<int>E<int>>   (ejemplo observado: <A1B2C3D4E5>)
  Se publican como valores enteros en topics individuales.

Comandos (RPi -> myRIO):
  <V<lin.x> <lin.y> <ang.z>>  (velocidad)
  <R<pos>>                   (cremallera)
  <G<0/1>>                   (gripper)
  <E>                        (emergencia)
  <P>                        (ping)
...
class UARTBridge(Node):
    def __init__(self):
        ...
        # Publicadores (valores enteros por simplicidad, se pueden cambiar a Float32 si es necesario)
        ...
        self.get_logger().info("UART Bridge (formato <...>) iniciado.")

    # ------------------------------------------------------------------
    # Envío de comandos con delimitadores <>
    # ------------------------------------------------------------------
